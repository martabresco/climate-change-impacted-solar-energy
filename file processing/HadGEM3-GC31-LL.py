...
def main(): 
    ...
    folder_path = "/groups/FutureWind/SFCRAD/HadGEM3-GC31-MM/ssp585/r1i1p1f3/" #remember to change depending on model
    ...
    def find_overlaps(list1, list2):
        ...
        for start1, end1, file1 in list1:
            for start2, end2, file2 in list2:
                # Convert to datetime objects if using cftime
                if isinstance(start1, cftime.Datetime360Day):
                    start1 = cftime.datetime_to_datetime(start1)
                if isinstance(end1, cftime.Datetime360Day):
                    end1 = cftime.datetime_to_datetime(end1)
                if isinstance(start2, cftime.Datetime360Day):
                    start2 = cftime.datetime_to_datetime(start2)
                if isinstance(end2, cftime.Datetime360Day):
                    end2 = cftime.datetime_to_datetime(end2)
    ...
    for overlap_start, overlap_end, tas_file, rsds_file, rsdsdiff_file in final_overlaps:
        ...
        if 'time' in ds.dims:
            ...
            for year in unique_years:
                ...
                output_file = f"/groups/FutureWind/SFCRAD/HadGEM3-GC31-MM/ssp585/r1i1p1f3/rsds_rsdsdiff_tas_{year}.nc" #remember to change depending on model
                
                # Check if the file already exists
                if not os.path.exists(output_file):
                    # Extract data for this year using slicing
                    yearly_data = ds.sel(time=slice(f"{year}-01-01", f"{year}-12-30"))
